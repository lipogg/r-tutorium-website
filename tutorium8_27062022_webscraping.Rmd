...
1. API der Website abrufen: Vielleicht verfügt die Website über eine Schnittstelle, eine sogenannte API, die ihr anzapfen könnt, um direkt die Dateien herunterzuladen. API steht für **A**pplication **P**rogramming **I**nterface und wird verwendet, um anderen den Zugriff auf Daten oder Funktionalitäten einer Anwendung zu ermöglichen, ohne dabei den Quellcode der Anwendung zu offenbaren. Die Inhalte können mit einer simplen Abfrage ("GET-Request") über das Internet-Protokoll HTTP angezapft werden. 
     + Beispiele: [dracor.org](https://dracor.org/doc/api), [Chronicling America Archive](https://chroniclingamerica.loc.gov/about/api/) 

2. Web Scraping: Wenn es keine API gibt, gibt es mehrere mögliche Strategien. Um euch für eine Strategie zu entscheiden, müsst ihr euch zunächst den Quelltext der Website ansehen. 
     + Ist die Website sehr einfach strukturiert, seht ihr den Inhalt der Seite meist direkt zwischen verschiedenen HTML-Tags. In diesem Fall könnt ihr den HTML-Baum parsen und den Inhalt der Elemente extrahieren, ähnlich wie wir das bereits für XML-Dateien gelernt haben. 
...
## 2. Dracor: Datenextraktion mithilfe der API

Die Dramen-Datenbank Dracor verfügt über eine [gut dokumentierte API](https://dracor.org/doc/api), welche den Zugriff auf verschiedene Inhalte ermöglicht, beispielsweise auf Metadaten zu Stücken oder ganzen Korpora, auf den Sprechtext der Figuren im Plaintext-Format und auch auf ganze, in XML-TEI ausgezeichnete Stücke. Die Abfrage dieser Daten für einzelne Stücke ist direkt über das Webinterface möglich. Um Daten zu mehreren Stücken zu bekommen, müssen wir ein Skript schreiben, das die API anzapft und die URL für die API-Abfrage iterativ (also in mehreren Durchläufen) für jedes der uns interessierenden Stücke anpasst.
...
## 3. Projekt Gutenberg: HTML-Baum parsen 

Die Open Source - Volltextdatenbank [Projekt Gutenberg](https://www.projekt-gutenberg.org) (nicht zu verwechseln mit dem englischsprachigen project-gutenberg.org!) verfügt über keine API. Dafür ist der Aufbau der Seite unkompliziert und auch mit nur rudimentärem Verständnis von HTML und CSS intuitiv verständlich. 
